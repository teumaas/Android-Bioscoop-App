...
import nl.avans.informatica.bioscoopapp.util.OnShowAvailable;

/**
 * Created by sjoer on 24-3-2018.
 */

public class GetShowsRequest extends AsyncTask<String, Void, String> {
    private final static String TAG = "GetShowsRequest";
    private OnShowAvailable listener = null;

    public GetShowsRequest(OnShowAvailable listener){
    ...
    public void onPostExecute(String response) {
        ...
        try {
            ...
            JSONArray shows = jsonObject.getJSONArray("Shows");

            for(int idx = 0; idx < shows.length(); idx++){
                JSONObject Show = new JSONObject(shows.getString(idx));

                int ShowId = Show.getInt("ShowId");
                String Date = Show.getString("Date");
                String Time = Show.getString("Time");
                double Price = Show.getDouble("Price");
                String MovieId = Show.getString("MovieId");
                int RoomId = Show.getInt("RoomId");

                Show ShowObject = new Show(ShowId, Date, Time, Price, MovieId, RoomId);

                listener.onShowAvailable(ShowObject);
            }
